...
echo "Public Node Public IP: ${aws_instance.public-node.public_ip[0]}"

echo "System Gateway Public IP: ${aws_instance.system-gateway.public_ip}" > "${path.module}/output/${var.cluster-name}.out"
echo "Public Node Public IP: ${aws_instance.public-node.public_ip[0]}" >> "${path.module}/output/${var.cluster-name}.out"
...
${local.worker-node-userdata}
/etc/eks/bootstrap.sh --kubelet-extra-args --node-labels=nodetype=stateful,nodegroup=public-apps,node-ip=$NODE_IP --apiserver-endpoint '${aws_eks_cluster.eks-cluster.endpoint}' --b64-cluster-ca '${aws_eks_cluster.eks-cluster.certificate_authority.0.data}' '${var.cluster-name}'
USERDATA
}
